...
- 拟人化新增**过程进度提示开关（progressStatus，默认开启）**：处理任务时显示中间状态气泡（如"正在处理…""正在使用工具…""正在整理结果…"）。关闭后不再发送占位气泡与中间进度文案，回复直达，流式文本仍正常逐字显示。**与 message_break 同时开启时**，关闭此项会连带跳过占位流（分段直接逐条发送，消除占位气泡与分段消息的内容重复）；纯流式时以"懒建流"模式延迟到首条真实文本才创建消息，不发占位。支持所有聊天渠道（QQ/微信本就无中间进度，AI Office 任务进度不受影响）。支持每机器人覆盖，改设置下一条消息即生效，无需重启。
  Humanization adds a **progress hints toggle (progressStatus, on by default)**: interim status bubbles shown while processing (e.g. "Processing…", "Using tool…", "Gathering results…") can now be turned off so no placeholder or progress text is sent — replies arrive directly while streaming text still appears word by word. **When message_break is also on**, turning this off additionally skips the placeholder stream entirely (segments are sent directly, removing the duplicate top bubble); in pure streaming mode a "lazy stream" defers message creation until the first real text, so no placeholder is ever sent. Applies to all chat channels (QQ/WeChat already have no interim progress; AI Office job progress is unaffected). Supports per-bot overrides and takes effect on the next message without a restart.
- 拟人化设置支持**逐 bot 逐项覆盖全部设置**：每个渠道机器人卡片的「拟人化」折叠面板现可覆盖全部 8 项设置（流式回复、分步消息、状态表情回应、回复引用、新消息行为、输入状态指示、断续节奏、发送延迟），不再仅限发送延迟。每项独立选择「跟随全局」或自定义值，保存只写被覆盖的项，未覆盖项继续跟随全局默认变化。后端数据模型此前已支持逐键替换，本次开放 UI。
  Humanization settings now support **per-bot per-key override for ALL settings**: each channel bot card's "Humanization" panel can override all 8 settings (streaming, message break, status reaction, reply quote, on-new-message behavior, typing indicator, burst rhythm, send delay), not just send delay. Each setting independently chooses "follow global" or a custom value; only overridden settings are saved, while the rest continue to track the global default. The backend data model already supported per-key replacement; this release opens the UI.
- 全局面板与逐 bot 编辑器改为渲染**同一套共享字段模块**（`humanize-fields.js`：字段布局、提示文案、探索预设、上限与校验错误单一来源），两个界面的详略度与格式完全一致，不会再各自漂移；全局面板的发送延迟/断续节奏同步获得探索预设与保存时校验（字段错误就地显示），行为与逐 bot 编辑器对齐。全局面板从 590 行降到 273 行，逐 bot 编辑器从 810 行降到 419 行。
  The global panel and the per-bot editor now render from **one shared field module** (`humanize-fields.js`: a single source for field layout, hints, exploration presets, limits, and validation errors), so the two surfaces are identical in detail and format and can no longer drift apart; the global panel's send-delay/burst fields gain the exploration presets and on-save validation (inline field errors), matching the per-bot editor's behavior. The global panel shrank from 590 to 273 lines; the per-bot editor from 810 to 419.
- 拟人化新增两个全局开关，对应原始诉求「关掉 Telegram 的 reaction（点赞、眼睛）和回复引用」：**状态表情回应（statusReaction，默认开启）**关闭后六渠道（Telegram/Slack/Discord/WhatsApp/飞书/钉钉）不再发送处理中/成功/失败表情，回复照常送达；**回复引用（replyQuote，默认开启）**关闭后机器人回复不再以「回复/引用」形式指向用户消息，仅影响 Telegram/Discord/WhatsApp 的引用样式，话题路由与线程/话题归组不变。WhatsApp 的已读回执不受影响（门控在发送层，保留 `quoted.key` 用于已读标记）。两项均支持每机器人覆盖，改设置下一条消息即生效，无需重启。**升级兼容**：新客户端写旧宿主会被 `SETTABLE_KEYS` 整体拒绝（保存报错，非部分成功）——客户端与宿主需同版本；旧宿主读新 humanize.json 安全。
  Humanization adds two global toggles, answering the original request "turn off Telegram reactions (👍/👀) and reply quotes": **Status emoji reactions (statusReaction, on by default)** — when off, none of the six channels (Telegram/Slack/Discord/WhatsApp/Feishu/DingTalk) send processing/success/failure emoji and replies still deliver; **Reply quotes (replyQuote, on by default)** — when off, the bot's replies no longer quote the user's message, affecting only the quote style on Telegram/Discord/WhatsApp; topic routing and thread/topic grouping are unchanged. WhatsApp read receipts are unaffected (gated at the send layer; `quoted.key` is still used for read markers). Both support per-bot overrides (data model, no per-bot UI control) and take effect on the next message without a restart. **Upgrade note**: a new client writing to an older host is rejected wholesale by `SETTABLE_KEYS` (the save fails rather than partially succeeding) — client and host must be the same version; an older host reading a new humanize.json is safe.
- 拟人化两阶段发送：新增**发送延迟**（默认关闭）与**输入状态指示**。阅读延迟在回合开始前静默 `readDelay`（min/max 随机 + 可选按用户消息长度的阅读项，`maxTotalMs` 封顶；无输入状态接口的渠道封顶 5 秒），**活跃响应**（activityBoost）让刚聊完天的回合"秒回"：上一回合结束后秒回窗口（默认 1 分钟）内直接用快速回复（默认约 1 秒），恢复下限窗口（默认 2 分钟）前线性回升，超过完全恢复窗口（默认 5 分钟）回到完整随机区间；首条消息不加速。分段间隔为分步消息/流式分段之间"正在打下一条"的停顿。延迟期间的新消息（interrupt）或 `/stop` 会**静默取代**旧回合：不生成、不发送、无失败提示、无双回复；排队模式延迟累积。发送延迟支持**按机器人覆盖**（渠道卡片内跟随全局/自定义），配置文件暴露全部参数（charsPerSecond、activityBoost、typingBurst 等）。
  Two-phase humanized sending: adds a **send delay** (off by default) and a **typing indicator**. A read delay stays silent before the turn starts (`readDelay` min/max random + optional per-message-length reading term, capped by `maxTotalMs`; channels without a typing API are capped at 5 seconds), and the **activity boost** (activityBoost) makes a turn right after a quick exchange "reply in seconds": within the fast-reply window (default 1 min) after the previous turn ends use the fast reply (default ~1 s) directly, ramp linearly back to the floor by the floor-recovery window (default 2 min), and past the full-recovery window (default 5 min) return to the full random range; the first message is never accelerated. Segment gaps pause between message-break or streamed segments like someone typing the next message. A new message (interrupt) or `/stop` during the delay **silently supersedes** the old turn — no generation, no send, no failure notice, no double reply; queued messages accumulate delays. The send delay supports **per-bot overrides** (follow global / custom override on each channel card), and the config file exposes every parameter (charsPerSecond, activityBoost, typingBurst, …).
...
- **breaking（配置）**：`readDelay.idleBoost`（闲置加成：闲置越久延迟 ×N）被语义反转并替换为 `readDelay.activityBoost`（活跃响应：刚聊完天"秒回"）。已存的 `idleBoost` 键在读取时被忽略并回落新默认值——如果你之前调过闲置加成，请在设置面板重配活跃响应。
  **Breaking (config)**: `readDelay.idleBoost` (idle boost — the longer the idle, the slower the reply) is semantically inverted and replaced by `readDelay.activityBoost` (active response — fast replies right after a quick exchange). A stored `idleBoost` key is ignored on load and falls back to the new defaults; if you had tuned the idle boost, re-configure the activity boost in the settings panel.
...
> 本仓库（humanize fork）在 [Unreleased] 段维护拟人化（humanize）功能层的变更条目；其下为上游 [xmanrui/dsh-im](https://github.com/xmanrui/dsh-im) 的版本历史。
