...
function GroupResponseModeEditor({
  ...
  const [authorizing, setAuthorizing] = React21.useState(false);
...
function FeishuGroupSettingsPage({ account, rpcCall }) {
  const [settings, setSettings] = React21.useState(() => groupSettingsFrom(account));
  const [refreshing, setRefreshing] = React21.useState(false);
  const [refreshError, setRefreshError] = React21.useState(null);
  const [provision, setProvision] = React21.useState(null);
  const [provisionBusy, setProvisionBusy] = React21.useState(false);
  ...
  const applySnapshot = React21.useCallback((value) => {
    const result = targetBotFromSnapshot(value, account.botId);
    if (mounted.current) setSettings(groupSettingsFrom(result.bot));
    return result;
  }, [account.botId]);
  const loadSettings = React21.useCallback(async ({ signal, restoreProvisioning = false } = {}) => {
    setRefreshing(true);
  ...
  const saveSetting = React21.useCallback(async (endpoint, payload) => {
    const value = await invoke(endpoint, { botId: account.botId, ...payload });
    applySnapshot(value);
  }, [account.botId, applySnapshot, invoke]);
  const startAuthorization = React21.useCallback(async ({ replace = false } = {}) => {
  ...
  const finishAuthorization = React21.useCallback(async (signal) => {
  ...
  const cancelAuthorization = React21.useCallback(async () => {
...
function GlobalSettingsPanel({ rpcCall }) {
  ...
  const [loadError, setLoadError] = React23.useState(null);
  const [ttlInput, setTtlInput] = React23.useState("");
  const [savedTtl, setSavedTtl] = React23.useState(null);
  const [ttlError, setTtlError] = React23.useState(false);
  const [saveError, setSaveError] = React23.useState(null);
  const [saveSucceeded, setSaveSucceeded] = React23.useState(false);
  const [isSaving, setIsSaving] = React23.useState(false);
  const [sweepConfirming, setSweepConfirming] = React23.useState(false);
  const [sweeping, setSweeping] = React23.useState(false);
  const ttlErrorId = React23.useId();
  const ttlHintsId = React23.useId();
  const sweepTriggerId = React23.useId();
  const sweepConfirmId = React23.useId();
  const sweepConfirmTextId = React23.useId();
  const sweepConfirmButtonId = React23.useId();
  const mounted = React23.useRef(true);
  const saving = React23.useRef(false);
  ...
  const loadSettings = React23.useCallback(async ({ signal } = {}) => {
    setPhase("loading");
    setLoadError(null);
    try {
...
function IMSettingsTab({
  ...
  const generalSettingsTooltipId = React25.useId();
