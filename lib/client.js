...
var package_default = {
  ...
  dsh: {
    ...
    compatibility: {
      dsh: "0.1.2-alpha.4 || 0.1.2-alpha.5 || 0.1.2-rc.1",
      dshReleases: {
...
var DEFAULT_INBOUND_TTL_HOURS = 0;
...
var SWEEP_CONFIRM_RESET_MS = 8e3;
...
function GlobalSettingsPanel({ rpcCall }) {
  ...
  const [loadError, setLoadError] = React22.useState(null);
  const [ttlInput, setTtlInput] = React22.useState("");
  const [savedTtl, setSavedTtl] = React22.useState(null);
  const [ttlError, setTtlError] = React22.useState(false);
  const [saveError, setSaveError] = React22.useState(null);
  const [sweepConfirming, setSweepConfirming] = React22.useState(false);
  const [sweeping, setSweeping] = React22.useState(false);
  const ttlErrorId = React22.useId();
  const ttlHintsId = React22.useId();
  const sweepHintId = React22.useId();
  const mounted = React22.useRef(true);
  const saving = React22.useRef(false);
  const sweepResetTimer = React22.useRef(null);
  ...
  const loadSettings = React22.useCallback(async ({ signal } = {}) => {
    setPhase("loading");
    setLoadError(null);
    try {
      ...
      if (signal?.aborted || caught?.name === "AbortError" || !mounted.current) return;
      setLoadError(presentError11(caught, "\u65E0\u6CD5\u8BFB\u53D6\u5168\u5C40\u8BBE\u7F6E\uFF0C\u8BF7\u7A0D\u540E\u91CD\u8BD5\u3002"));
      setPhase("error");
    }
  }, [invoke]);
  React22.useEffect(() => {
  ...
  const inlineStatus = phase === "loading" ? { role: "status", message: "\u6B63\u5728\u8BFB\u53D6\u5168\u5C40\u8BBE\u7F6E\u2026" } : ttlError ? { role: "alert", message: `\u8BF7\u8F93\u5165 -1\u30010 \u6216 1~${INBOUND_TTL_MAX_HOURS} \u4E4B\u95F4\u7684\u6574\u6570\u3002` } : saveError ? { role: "alert", message: saveError } : phase === "error" && loadError ? { role: "alert", message: loadError } : null;
  return h2(
    "section",
    {
    ...
    h2(
      ...
      h2(
        "span",
        { className: "dim-globalHeadActions" },
        ...
        h2(GlobalButton, {
          ...
          disabled: sweeping,
          "aria-describedby": sweepConfirming ? sweepHintId : void 0
        }, sweeping ? "\u6B63\u5728\u6E05\u7406\u2026" : sweepConfirming ? "\u786E\u8BA4\u6E05\u7406" : "\u6E05\u7406\u8FC7\u671F\u9644\u4EF6")
      )
    ),
    h2(
      "ul",
      { id: ttlHintsId, className: "dim-globalTtlHints" },
      h2(
        "li",
        null,
        h2("code", null, "-1"),
...
.dim-rail .dim-channelGlobal { margin-bottom: 4px; }
.dim-logoGlobal { color: #fff; background: linear-gradient(145deg, #646a73, #343a46); }
.dim-logoGlobal svg { width: 19px; height: 19px; }
...
.dim-globalHead { min-width: 0; display: flex; align-items: center; justify-content: space-between; gap: 12px; }
...
.dim-globalHeadActions { flex: none; display: inline-flex; align-items: center; gap: 7px; }
.dim-globalTtlHints { display: grid; gap: 3px; margin: 10px 0 0; padding: 8px 10px; border: 1px solid var(--dsw-alias-border-l1, #eef0f3); border-radius: 9px; background: var(--dsw-alias-bg-module-platform, #f7f8fa); list-style: none; }
.dim-globalTtlHints li { min-width: 0; display: flex; align-items: baseline; gap: 8px; }
.dim-globalTtlHints code { flex: none; min-width: 44px; padding: 0 6px; border-radius: 5px; color: var(--dsw-alias-label-primary, #1f2329); background: var(--dsw-alias-bg-layer-1, #fff); font: 10px/16px ui-monospace, SFMono-Regular, Menlo, monospace; text-align: center; }
...
.dim-globalTtlInput { width: min(160px, 100%); min-width: 0; height: 34px; padding: 0 11px; border: 1px solid var(--dsw-alias-border-l2, #dfe1e5); border-radius: 8px; color: var(--dsw-alias-label-primary, #1f2329); background: var(--dsw-alias-bg-layer-1, #fff); font: 13px ui-monospace, SFMono-Regular, Menlo, monospace; transition: border-color .16s ease, box-shadow .16s ease; }
...
.dim-globalInline { flex: 1 1 120px; min-width: 0; margin: 0; color: var(--dsw-alias-label-secondary, #646a73); font-size: 11px; line-height: 16px; overflow-wrap: anywhere; }
.dim-globalInline[data-tone="error"] { color: var(--dsw-alias-state-error-primary, #d54941); }
.dim-globalScreenReader { position: absolute; width: 1px; height: 1px; margin: -1px; padding: 0; border: 0; clip: rect(0 0 0 0); clip-path: inset(50%); overflow: hidden; white-space: nowrap; }
...
function IMSettingsTab({
  ...
  const activeTabId = globalSettingsSelected ? `dim-tab-${GLOBAL_SETTINGS_TAB_ID}` : `dim-tab-${active.id}`;
      ...
      h2(
        ...
        h2(
          ...
          h2(
            ...
            h2(
              "span",
              { className: "dim-logo dim-logoGlobal", "aria-hidden": "true" },
              h2(GlobalSettingsLogoGlyph)
            ),
            h2(
              "span",
