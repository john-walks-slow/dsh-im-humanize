...
export function validateHumanizeOverrideSection(value, { sendDelayBase = null } = {}) {
  ...
  if (value.statusReaction !== undefined) {
    if (typeof value.statusReaction !== 'boolean') {
      throw invalid('humanize.statusReaction', 'statusReaction must be a boolean.');
    }
    out.statusReaction = value.statusReaction;
  }
  if (value.replyQuote !== undefined) {
    if (typeof value.replyQuote !== 'boolean') {
      throw invalid('humanize.replyQuote', 'replyQuote must be a boolean.');
    }
