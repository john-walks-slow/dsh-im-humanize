...
**分步直推（Step Push）**：
开启后回合内的工具调用与助手中间说明以独立消息逐条推送到会话路由的呈现模式；最终答案仍以流式卡呈现。它是详细级别过程呈现的开关，默认关闭保持原流式模式。
_Avoid_: 离散模式、逐条刷屏、过程广播

**交互后换卡（Post-Interaction Rotation）**：
交互消息（提问/审批）呈现前后，流式卡把旧卡定格为过程记录并把后续过程更新与最终答案归属到交互消息下方新卡的呈现规则。
_Avoid_: 换消息、重开回合、卡片重置

**换卡挂起（Rotation Hold）**：
交互后换卡中，旧卡定格后到交互消息呈现完成前，暂缓创建新卡、新到过程快照只保留最新一条的状态；呈现完成后才惰性建卡，保证新卡位于交互消息下方。
_Avoid_: 写入暂停、卡片锁定、挂起回合

**冻结前缀去重（Frozen-Prefix Dedup）**：
交互后换卡中，以定格时旧卡展示的内容为基线，后续更新与终稿分段剥离与之逐字一致的前缀、只展示增量的去重规则；前缀不匹配则原样写入。
_Avoid_: 内容过滤、历史裁剪、快照去重

**已答状态卡（Answered-State Card）**：
提问卡被回答后原地替换为的回执样式：保留问题与选项文本、标注已选项、不再提供任何可点按钮；替换失败明确降级，不影响答案提交。
_Avoid_: 按钮置灰补丁、二次弹窗、消息撤回
