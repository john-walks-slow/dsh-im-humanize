...
2. 给本机 iMessage 身份发送测试消息的联系人，必须是**另一个外部 iMessage 地址**，例如另一部 iPhone 使用的电话号码或 Apple ID。
3. 在 Harness 的 IM 机器人设置页打开 iMessage，点击“配置本机权限”，完成两项授权。
4. 点击“检查权限并启用”，确认状态显示为已连接后，再从外部联系人发送一条普通文本。
5. 等待 Harness 回复；不要同时把机器人发送目标设置成本机正在使用的同一身份。

使用同一个本机身份给自己发消息可能造成自聊回环：机器人把自己的回复再次当作新输入，产生重复回复或发送失败。出现这种情况时，应先停止 iMessage 机器人，清理对应会话映射，再使用外部联系人重新测试。
